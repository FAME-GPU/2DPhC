...
lattice_vec = 0.5*a*[1.0, 1.0; -sqrt(3.0), sqrt(3.0)];
% u1 = lattice_vec(:,1);
% u2 = lattice_vec(:,2); 
...
eps1 = [12.0, 1i*kappa1, 0.0; -1i*kappa1, 12.0, 0.0; 0.0, 0.0, 5.0];
mu1 = [4.0, 1i*kappa2, 0.0; -1i*kappa2, 4.0, 0.0; 0.0, 0.0, 2.0];

parameter.center = [xc, yc]*lattice_vec';
parameter.func_type = 'circle';
parameter.r  = 0.3*a;
...
[kx_coord, ky_coord, numerator, BZvertex_string, vertex_ind_Xcoord, ...
        GXSY_idx, k_point_repeatind] = kpath_hex(k_grid_num);
...
tmp = diff(omega_tm)./omega_tm(2:end,:);
tmp = abs(tmp) < opts.tol;
